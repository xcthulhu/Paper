...
\newcommand{\rewrite}[2][]
{\todo[caption={#2}, color=green!40, #1]{\renewcommand{\baselinestretch}{1}\selectfont#2\par}}
\newcommand{\error}[2][]
...
\otodo{First order
  intuitionistic logic also has that $\vdash \exists x F x \iff \vdash
...
quantifier.\rewrite{This paragraph is important, so I rewrote it
...
\rewrite{Aren't theories closed under deduction?  Also, ``theories''
  are a proof theoretic concept; your paper is about semantics
  so it may be prudent to caste this discussion in terms models or
  classes of models.} Quine's criteria of ontological commitment %will be understood as a
...
which an individual adheres.\rewrite{Defending the standard, logic based understanding
  of theories like this is not very relevant to your paper.  Delete
